...
**Files & results** shows where outputs are saved. Unfinished bundles are clearly
labelled as incomplete. ZIP import and guided CSV/Parquet conversion are not included.
...
`MEDDEID_WORKSPACE_DIR=/app/data/workspaces`. Published 0.3.1 images include
this interface.
...
```bash
docker pull ghcr.io/stighellemans/meddeid-annotate:0.3.1
mkdir -p data
